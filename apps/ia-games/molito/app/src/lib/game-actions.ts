...
export type PlaceResult = { ok: true; mill: boolean } | { ok: false; error: string }

export type RemoveResult = { ok: true } | { ok: false; error: string }

export type MoveResult = { ok: true; mill: boolean } | { ok: false; error: string }

export function placePiece(game: Game, position: number, playerKey: PlayerKey): PlaceResult {
...
export function removePiece(game: Game, position: number, playerKey: PlayerKey): RemoveResult {
  if (game.status !== 'playing') {
    return { ok: false, error: 'La partida no esta en curso' }
  }
...
export function movePiece(game: Game, from: number, to: number, playerKey: PlayerKey): MoveResult {
  if (game.status !== 'playing') {
    return { ok: false, error: 'La partida no esta en curso' }
  }
