...
describe('WaitingRoom', () => {
  ...
  it('shows waiting indicator', () => {
    renderWaitingRoom()
    expect(document.getElementById('waiting-text')!.textContent).toBe('Esperando jugador...')
  })
  ...
  it('copies the URL when clicking Copiar link', async () => {
    ...
    await vi.waitFor(() => {
      expect(navigator.clipboard.writeText).toHaveBeenCalledWith('http://localhost/molino/AB3K')
    })
  })
