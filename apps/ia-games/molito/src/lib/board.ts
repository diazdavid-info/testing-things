 ...
 */
export function checkMill(board: BoardCell[], position: number, playerKey: PlayerKey): boolean {
 ...
 */
export function getRemovablePositions(board: BoardCell[], playerKey: PlayerKey): number[] {
 ...
 */
export function getFormedMills(board: BoardCell[], position: number, playerKey: PlayerKey): number[][] {
  const result: number[][] = []
  for (const millIdx of POSITION_MILLS[position]) {
